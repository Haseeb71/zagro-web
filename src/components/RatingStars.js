  ...
  const renderStar = (index) => {
    ...
    if (difference >= 1) {
      ...
      return (
        <svg
          key={index}
          className={`${sizes[size]} text-yellow-400 transition-all duration-200 ${interactive ? 'hover:scale-110 cursor-pointer' : ''}`}
          fill="currentColor"
          viewBox="0 0 20 20"
      ...
      return (
        <div key={index} className="relative">
          <svg
            className={`${sizes[size]} text-gray-300 transition-all duration-200 ${interactive ? 'hover:scale-110 cursor-pointer' : ''}`}
            fill="currentColor"
            viewBox="0 0 20 20"
            onClick={() => interactive && onRatingChange?.(starValue)}
          >
